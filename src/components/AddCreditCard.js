...
const AddCreditCard = (props) => {
  ...
  const validate = () => {
    const { card_number, name_on_card, cvv, expiry_month, expiry_year } = formState;
    setError({
      ...error,
    ...
    if ((card_number && name_on_card && cvv && expiry_month && expiry_year) &&
      (!error.card_number && !error.name_on_card && !error.expiry_month && !error.expiry_year && !error.cvv)){
      return true;
    } else {
      return false;
    }
        ...
        <div className="credit-card-form">
          <Form>
            ...
            <Form.Group as={Row} controlId="formHorizontalEmail">
              ...
              <Col sm={6}>
              <DatePicker picker="month" className="form-control" onChange={get_date} defaultValue={moment("12-2024" , "MM-YYYY")} />
              </Col>
              <Col sm={6}>
                <Form.Control
